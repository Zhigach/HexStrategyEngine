...
/* This is a companion class to HexPresenter.java. It handles all of the mechanics related to hexagon grids. */
...
//I think that this XYvertex stuff is taken care of in the int x line above. Why is it here twice?
    ...
    /********************************************************************
     ...
     The hexagon is drawn in the colour specified in HexPresenter.COLOURELL.
     *********************************************************************/
    public static void drawHex(int i, int j, Graphics2D g2) {
        int x = i * (s + t);
        int y = j * h + (i % 2) * h / 2;
        Polygon poly = hex(x, y);
        g2.setColor(HexPresenter.COLOURCELL);
        //g2.fillPolygon(hexmech.hex(x,y));
        g2.fillPolygon(poly);
        g2.setColor(HexPresenter.COLOURGRID);
        g2.drawPolygon(poly);
    }
     ...
     *Purpose: This draws a filled in polygon based on the coordinates of the hexagon.
     ...
     The colour is set by HexPresenter.COLOURONE and HexPresenter.COLOURTWO.
    ...
    public static void fillHex(int i, int j, int n, Graphics2D g2) {
        ...
        int y = j * h + (i % 2) * h / 2;
        if (n < 0) {
            g2.setColor(HexPresenter.COLOURONE);
            g2.fillPolygon(hex(x, y));
            g2.setColor(HexPresenter.COLOURONETXT);
            c = (char) (-n);
            g2.drawString("" + c, x + r + BORDERS, y + r + BORDERS + 4); //FIXME: handle XYVertex
            //g2.drawString(x+","+y, x+r+BORDERS, y+r+BORDERS+4);
        }
        if (n > 0) {
            g2.setColor(HexPresenter.COLOURTWO);
            g2.fillPolygon(hex(x, y));
            g2.setColor(HexPresenter.COLOURTWOTXT);
            c = (char) n;
            g2.drawString("" + c, x + r + BORDERS, y + r + BORDERS + 4); //FIXME handle XYVertex
